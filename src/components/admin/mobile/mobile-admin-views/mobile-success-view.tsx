...
interface MobileSuccessViewProps {
...
export const MobileSuccessView = ({
  displayQuests,
  isLoading,
  selectedQuests,
  onQuestToggle,
  ...
  return (
    <>
      <div className="mb-2 flex gap-x-2 px-1 text-[10px] font-medium text-black/50">
        ...
        <span className="w-16 pl-4">Title</span>
        <span className="w-12 text-center">End date</span>
        <span className="w-16">Total amount</span>
        <span className="w-12 text-end">Total vote</span>
      </div>
        ...
        <div className="space-y-2">
          {displayQuests.map(q => (
            ...
            >
              ...
              <span className="w-16 truncate pl-4 text-black">
                {q.quest_title}
              </span>
              <span className="w-12 text-center text-black">
                {dayjs(q.quest_end_date).format('YYYY/MM/DD')}
              </span>
              <span className="w-16 text-center text-black">
                {formatNumber(q.total_betting_amount, {
                  minimumFractionDigits: 0,
                  maximumFractionDigits: 2,
                })}
                {' '}
                {getBettingToken(q).symbol}
              </span>
              <span className="w-12 text-center text-black">
                {q.total_vote}
              </span>
